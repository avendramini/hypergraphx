...
def compute_directed_motifs(hypergraph: Hypergraph, order=3, runs_config_model=10):
    ...
    def _motifs_order_5(edges):
        full, visited = _directed_motifs_ho_full(edges, 5)
        not_full, visited = _directed_motifs_ho_not_full(edges, 5, visited)
        #standard = _motifs_standard(edges, 5, visited)
    ...
    if order == 3:
        ...
        output['observed']= _motifs_order_5(edges)
    else:
    ...
    for i in range(ROUNDS):
        ...
        if order == 3:
            ...
            m1= _motifs_order_5(e1)
        #print(len(m1))
        results.append(m1)

    output['config_model'] = results
    
    delta = diff_sum(output['observed'], output['config_model'],directed=True)
